...
function escaparTextoLocalesReactivacion(string $valor): string
{
...
<head>
    ...
    <title>ManGo! - Reactivar local</title>
...
<body>
    ...
    <main class="rdm--contenedor-toolbar">
        <?php if ($errores !== []): ?><aside class="rdm-alert rdm-alert--error" role="alert" aria-live="assertive"><div class="rdm-alert--icon"><span class="material-symbols-rounded">error</span></div><div class="rdm-alert--body"><?php if (count($errores) === 1): ?><p class="rdm-sys-typography--body-large"><?= escaparTextoLocalesReactivacion($errores[0]) ?></p><?php else: ?><ul class="rdm-sys-typography--body-large"><?php foreach ($errores as $error): ?><li><?= escaparTextoLocalesReactivacion($error) ?></li><?php endforeach; ?></ul><?php endif; ?></div></aside><?php endif; ?>

        <form class="rdm-form--container" method="post" action="index.php?accion=reactivar-local&id=<?= (int) $local['id'] ?>"><div class="rdm-form--elevated"><div class="rdm-form--body"><h2 class="rdm-sys-typography--display-small">Activar</h2><h3 class="rdm-sys-typography--title-large">¿Confirmar activación?</h3><p class="rdm-sys-typography--body-large">El local <strong><?= escaparTextoLocalesReactivacion((string) $local['nombre']) ?></strong> volverá a aparecer como disponible para operación.</p><input type="hidden" name="token_csrf" value="<?= escaparTextoLocalesReactivacion($tokenCsrf) ?>"><div class="rdm-textfield--wrapper"><div class="rdm-textfield--container rdm-textfield--outlined"><div class="rdm-textfield--control"><input type="password" id="contrasena_actual" name="contrasena_actual" placeholder=" " autocomplete="current-password" aria-describedby="contrasena_actual_support" required><label class="rdm-textfield--label" for="contrasena_actual">Contraseña actual</label></div></div><div class="rdm-textfield--support"><span class="rdm-textfield--support-text" id="contrasena_actual_support">Confirma tu identidad para continuar.</span></div></div></div><div class="rdm-form--action-left"><p><button class="rdm-button--filled" type="submit"><div class="rdm-button--container"><div class="rdm-button--media"><div class="rdm-button--icon"><span class="material-symbols-rounded">store</span></div></div><div class="rdm-button--body"><span class="rdm-sys-typography--label-large">Confirmar</span></div></div></button><button class="rdm-button--text" type="button" onclick="window.location.href='index.php?accion=editar-local&id=<?= (int) $local['id'] ?>';"><div class="rdm-button--container"><div class="rdm-button--body"><span class="rdm-sys-typography--label-large">Cancelar</span></div></div></button></p></div></div></form>
    </main>
</body>
</html>